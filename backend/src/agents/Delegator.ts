...
import { z } from 'zod';
import { BuilderTask } from './Builder.js';
...
export class Delegator extends BaseAgent {
   ...
   */
  constructor() {
    super(AgentTypes.DELEGATOR, 'A software architect', SYSTEM_MESSAGE);
  }
   ...
   */
  async getCodebaseContext(maxFiles = 20): Promise<string> {
    try {
      // Get repository files
      const files = await getRepoFiles('./test-bed/cell-source', [
        'node_modules/*',
        '.git/*',
        '*.log',
        '*.lock',
      ...
      const importantFiles = files
        ...
        .filter((file) => ['.ts', '.ts', '.tsx', '.tsx', '.tson'].some((ext) => file.endsWith(ext)))
        .slice(0, maxFiles);
      ...
      logMessage({
        source: this.id,
        content: `Error gathering codebase context: ${errorMessage}`,
   ...
   * @param message - The user's task request
   */
  async handleDelegatorTask(message: string): Promise<void> {
    try {
